...
import argparse
import typing
from utils.dlib_crop_face import dlib_crop_twice,dlib_crop_more_ROI
...
# 定义可用的剪裁方法
CROP_FUNC_DICT = {'dlib_crop_twice':dlib_crop_twice, 'mtcnn_crop':crop_face_by_mtcnn,'dlib_crop_more_ROI':dlib_crop_more_ROI}
...
def crop_one_sample(crop_face_func:typing.Callable,img_paths:typing.Sequence[Path],save_img_paths:typing.Sequence[Path]):
    ...
    for i,(img_path,save_img_path) in enumerate(zip(img_paths,save_img_paths)):
        ...
        if i==0:
            box=crop_face_func(rgb_img,img_path=img_path)
    ...
    with multiprocessing.Pool(args.workers) as pool:
        ...
        for x in df.itertuples(index=False):
            ...
            _=pool.apply_async(crop_func,
                args=(img_paths,save_img_paths),callback=update,error_callback=lambda xx: print(xx))
            res.append(_)
        [x.get() for x in res]
