...
import logging
logging.basicConfig(level=logging.INFO, format=' [%(filename)s:%(lineno)d] - %(message)s - %(asctime)s - %(name)s - %(levelname)s')

def get_print_logger(debug=False):
...
def extract_metrics(pattern_str:str,path_list:Iterable[Union[str,Path]],save_file_path:Path,*,extra_words=Union[str,None]):
    """
    ...
    if extra_words is not None:
        last_rows.append(extra_words)
    if save_file_path is not None:
        pd.Series([*[x.name for x in path_list],*last_rows]).to_csv(save_file_path,header=[str(chdir_path)])
    else: 
        for item in last_rows:
            print(item)
    return last_line

def extract_metrics_from_dir(dir_path:Union[str,Path]):
    re_path = 'fold(?P<fold>\d+)-epoch=(?P<epoch>\d+)-acc=(?P<acc>[.0-9]{6})-uf1=(?P<uf1>[.0-9]{6})-uar=(?P<uar>[.0-9]{6})'
    return extract_metrics(re_path, Path(
        dir_path).glob('*.ckpt'),None)
...
def send_email(addr_from, authorization_code, addr_to, smtp_server, head_from, head_to, head_subject, message):
    ...
    server1 = partial(smtplib.SMTP, host=smtp_server, port=25, timeout=3)
    server2 = partial(smtplib.SMTP_SSL, host=smtp_server, port=465, timeout=3)

    for index,build_server in enumerate([server1, server2]):
        try:
            server = build_server()
    ...
    # 如果都失败了，可以选择抛出异常或者其他处理方式
    else:
        # raise Exception("All methods failed.")
        print('邮件超时未发送，可能是网络原因，或者端口等设置不正确')
    return False
...
def plot_confusion(y_true:list, y_pred:list, labels:Union[list,None],*,save_path: typing.Optional[Path]=None ):
    ...
    # calculate the uf1-score using sklearn
